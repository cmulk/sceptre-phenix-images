...
_minirouter_img: # use _img to avoid conflict with minirouter binary
	@$(CHECK_IMAGE)
	@$(PHENIX) image create -O $(CURDIR)/overlays/minirouter -P dnsmasq,openvswitch-switch,bird,nano,iptables,nftables,isc-dhcp-client,isc-dhcp-common -r noble $(COMPRESS) $(@)
	@$(PHENIX_IMAGE_BUILD) $(@)
	@$(INJECT_MINICCC)
